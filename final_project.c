...
bool woolf(int *years_left, int numbers[]) {
    system("clear");
    char *names[6] = {"0) Bernard", "1) Neville", "2) Louis", "3) Jinny", "4) Susan", "5) Rhoda"}; 
    int character;
    ...
    scanf("%d", &character);
    system("clear");
    printf("\n \n We need you to pick a character from The Waves. This character will be the fulcrum for the construction of your interpretation. Please type the number that corresponds to one of the characters listed below: \n"); 
    for(int i = 0; i <= 5; i++) { 
        if(numbers[i] == 1) { 
            printf("%s\n", names[i]); 
        }
